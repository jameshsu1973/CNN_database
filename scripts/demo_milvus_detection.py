"""
Database-Driven Object Detection Demo: Cat Detection (Zilliz Cloud/Milvus)

This demo shows:
1. First Training: Cat detection, cold start with random initialization
2. Store weights to Zilliz Cloud vault after training
...
This version uses Zilliz Cloud (Milvus) instead of local ChromaDB.
The vault is stored in the cloud and can be shared across multiple users.
...
def first_training_run(vault, category="cat", num_epochs=5):
    ...
    print(f"\n[5] Storing weights to Zilliz Cloud vault...")
...
def main():
    ...
    print("(Zilliz Cloud / Milvus Version)")
    print("="*70)
    print("\nThis demo shows:")
    print("  1. First training: Cat detection, cold start (He init)")
    print("  2. Store trained weights to Zilliz Cloud vault (tagged as 'cat')")
    print("  3. Second training: Same cat task, vault-initialized from Zilliz Cloud")
    ...
    if not milvus_uri or not milvus_token:
        print("\n" + "="*70)
        print("ERROR: Zilliz Cloud configuration not found!")
        print("="*70)
        print("\nPlease configure your Zilliz Cloud credentials:")
        ...
        print("     vector_db:")
        print("       milvus:")
        print("         uri: \"https://your-cluster.zillizcloud.com:19530\"")
        print("         token: \"your-api-key\"")
        print("")
        print("  Or set environment variables:")
        print("     export CNN_MILVUS_URI=\"https://...\"")
        print("     export CNN_MILVUS_TOKEN=\"your-api-key\"")
        print("")
        return

    print(f"\nUsing Zilliz Cloud configuration:")
    print(f"  URI: {milvus_uri}")
    print(f"  Similarity threshold: {config.similarity_threshold}")

    # Create Milvus vault (Zilliz Cloud)
    vault = MilvusWeightVault(
        uri=milvus_uri,
        token=milvus_token,
        collection_name=config.get('vector_db.milvus.collection_name', 'cnn_weights'),
        similarity_threshold=0.3,
        top_k_ratio=0.3
    )

    # Test connection
    print("\n[0] Testing connection to Zilliz Cloud...")
    try:
    ...
    # Show vault stats
    print("\n" + "="*70)
    print("ZILLIZ CLOUD VAULT STATUS AFTER FIRST TRAINING")
    print("="*70)
    stats = vault.get_stats()
...
1. First Training (Cold Start):
   ...
   - Weights stored to Zilliz Cloud vault (tagged as 'cat')
...
3. Benefits of Zilliz Cloud:
   - Data stored in cloud, accessible from anywhere
   - Multiple users can share the same vault
   - No local storage needed
   - Free tier available (5GB storage, 2.5M compute units/month)
